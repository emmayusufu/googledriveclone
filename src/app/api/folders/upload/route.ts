...
export async function POST(req: Request) {
  ...
  for (const file of files) {
    try {
      ...
      // Get the correct Cloudinary folder path from the parent folder if specified
      let folderPath = userId;

      if (parentId) {
        const parentFolder = await Folder.findById(parentId);
        if (parentFolder && parentFolder.cloudinaryPath) {
          folderPath = parentFolder.cloudinaryPath;
        } else {
          folderPath = `uploads/${userId}/${parentId}`;
        }
      } else {
